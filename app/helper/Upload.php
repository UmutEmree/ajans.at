...
class Upload {
	...
	public function yukle($isim,$yol,$type = 'image/*'){
		  if ( $this->uploaded ) {

       
            
		  $this->file_new_name_body = $isim;
		  $this->file_overwrite = true;
		  $this->allowed = array ($type);
		...
		public function yukle_th($isim,$yol,$boyut=false,$boyutlar="-"){
		  if ( $this->uploaded ) {
		  ...
		  $this->allowed = array ( 'image/*' );
		  		...
		  		if($boyut==true){
			foreach($boyutlar as $byt){
				$deger = explode("x",$byt);
				//resmin istediğimiz boyutlarda gönderilmesi ve o boyutlara watermak'ın uygulanması
			
				$this->image_resize = true;
				$this->image_ratio_crop = true;
				$this->image_x = $deger[0];
				$this->image_y = $deger[1];
				$this->file_new_name_body = $byt."_".$isim;
				$this->process($yol);
			}
		}
		   $this->file_new_name_body = $isim;
...
function resim_gonder($input,$ad,$yol,$boyut=false,$boyutlar="-",$watermark="-",$wmKonum="-"){

	/*  
	
		$input: formda input kısmına verilen 'name'
		$ad: resmin yüklendikten sonra değiştirileceği isim
		$yol: resmin dosya yolu
		$boyut: aşağıda 'false' veya '0' ise resim sadece orjinal boyutta gönderilir
		$boyutlar: resim gönderildikten sonra kırpılacak boyutlar (aşağıda)
		$watermark: resim üzerine eklenecek küçük logo
		$wmKonum: logonun resmin üzerindeki konumu
	
	*/


	$image = new upload($_FILES[$input]);
	if ($image->uploaded) {
		
		$image->file_new_name_body = $ad;
		
		// sadece resim formatları yüklensin
        $image->allowed = array ( 'image/*' );
		
		//orjinal resimde de watermark kullanılsın
		//$image->image_watermark = $watermark;
		//$image->image_watermark_position = $wmKonum;
		
		//resim orjinal boyutta da gönderilsin. 
		/*Not: resme uygulanan niteliklerin aktif olması için, bu fonksiyon en altta olmalı */
		//$image->process($yol."/original");
		...
		if($boyut==true){
			foreach($boyutlar as $byt){
				...
				$image->image_x = $deger[0];
				$image->image_y = $deger[1];
				$image->file_new_name_body = $ad."_".$byt;
				$image->process($yol);
			}
		}
		...
		//resim gönderildiyse...
		if ($image->processed) {
			$resim =  '<div class="alert alert-success"><b>'.$image->file_src_name.'</b> Adlı Dosya Yüklendi</div>
			<span style="font-size:14px;"> Dosya Adresi :  <a href="'.$image->file_dst_pathname.'">İndir</a></span>';
			$image->clean();
		} else {
			$resim = 'hata : ' . $image->error;
		}
	}
